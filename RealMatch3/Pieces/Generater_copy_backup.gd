...
#func check_match(cood):
#	var column = cood.x
#	var row = cood.y
#	var res = false
#	if !check_exist(cood) or all_pieces[column][row].matched == true:
#		return res
#	var current_color = all_pieces[column][row].color
#	if check_exist(cood + Vector2(-1, 0)) and check_exist(cood + Vector2(1, 0)):
#		if (Til.id_to_name[Til.get_cellv(cood + Vector2(-1, 0))][0] == "0"
#			or Til.id_to_name[Til.get_cellv(cood + Vector2(1, 0))][0] == "0"):
#			res = false
#		elif !all_pieces[column - 1][row].is_moving and !all_pieces[column + 1][row].is_moving:
#			if all_pieces[column - 1][row].color == current_color and all_pieces[column + 1][row].color == current_color:
#				all_pieces[column - 1][row].h_matched += 1
#				all_pieces[column - 1][row].dim()
#				all_pieces[column][row].matched = true
#				all_pieces[column][row].h_matched += 1
#				all_pieces[column][row].dim()
#				all_pieces[column + 1][row].h_matched += 1
#				all_pieces[column + 1][row].dim()
#				res = true
#	if check_exist(cood + Vector2(0, -1)) and check_exist(cood + Vector2(0, 1)):
#		if (Til.id_to_name[Til.get_cellv(cood + Vector2(0, -1))][0] == "0"
#			or Til.id_to_name[Til.get_cellv(cood + Vector2(0, 1))][0] == "0"):
#			res = false
#		elif !all_pieces[column][row - 1].is_moving and !all_pieces[column][row + 1].is_moving:
#			if all_pieces[column][row - 1].color == current_color and all_pieces[column][row + 1].color == current_color:
#				all_pieces[column][row - 1].v_matched += 1
#				all_pieces[column][row - 1].dim()
#				all_pieces[column][row].matched = true
#				all_pieces[column][row].v_matched += 1
#				all_pieces[column][row].dim()
#				all_pieces[column][row + 1].v_matched += 1
#				all_pieces[column][row + 1].dim()
#				res = true
...
func bomb_explosion(cood):
	...
	if piece.superpower:
		...
		var par = get_node("piece")
#		par.emitting = true
		print(par, piece.triger, piece.fire)
		piece.queue_free()
	pass
